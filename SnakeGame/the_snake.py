...
class Snake:
    ...
    def in_game(self, ai_settings, screen):
        ...
        if difficulty == "hard":
            ...
            if self.rect.x >= ai_settings.screen_width-20:
                #self.rect.x = 0
            ...
            if self.rect.y >= ai_settings.screen_height-12:
                #self.rect.y = 0
                self.reset(ai_settings, screen)
            elif self.rect.y < 0 - 4:
                #self.rect.y = ai_settings.screen_height
                self.reset(ai_settings, screen)

    def draw_snake(self, screen, ai_settings):
        for i in range(len(self.snake_list)):
            pygame.draw.rect(screen, ai_settings.snake_color, [self.snake_list[i][0], self.snake_list[i][1], ai_settings.snake_width, ai_settings.snake_height])
            # x = snake_list[i][0]
            # y = snake_list[i][1]
    ...
    def reset(self, ai_settings, screen):
        ...
        ai_settings.score = 0
        gf.message("Game Over!", ai_settings.message_color, ai_settings, screen, 2.5, 2.5)
